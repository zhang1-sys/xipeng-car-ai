...
function pickRecommendedPackages(config, message) {
  const text = String(message || "");
  const packages = [];
  const smartPack = config.packages.find((item) => /智驾/i.test(item.name));
  const comfortPack = config.packages.find((item) => /舒享|豪华/i.test(item.name));
...
function updateConfigStateFromMessage(state, message) {
  ...
  if (variant) nextState.selectedVariant = variant.name;

  const color = findOption(config.colors, message);
  if (color) nextState.selectedColor = color.name;

  const interior = findOption(config.interiors, message);
  ...
  if (shouldAutoRecommend(message)) {
    ...
    if (!nextState.selectedColor) nextState.selectedColor = config.colors[0]?.name || null;
    if (!nextState.selectedInterior) nextState.selectedInterior = config.interiors[0]?.name || null;
...
function getConfiguratorStage(configState) {
  ...
  if (configState.selectedInterior) return "interior_selection";
...
function buildChecklist(state) {
  return [
    ...
    { label: "确定外观颜色", done: Boolean(state.selectedColor) },
    { label: "确定内饰颜色", done: Boolean(state.selectedInterior) },
    { label: "确定选装包", done: Boolean(state.selectedPackages?.length) || Boolean(state.done) },
...
function buildChoices(state) {
  ...
  return {
    ...
    colors: (config?.colors || []).map((item) => ({
      name: item.name,
      premium: item.premium ?? 0,
    })),
    interiors: (config?.interiors || []).map((item) => ({
      name: item.name,
      premium: item.premium ?? 0,
    })),
...
function buildNextPrompt(state, message) {
  ...
  if (!config) {
    return {
      reply: "我先没识别到对应的车型配置规则。你可以直接说想配置 G6、G9、P7i 或 MONA M03。",
  ...
  if (!state.selectedVariant) {
    ...
    const lines = config.variants.map((item) => `- ${item.name}：${formatPrice(item.price)}，${item.highlight}`);
    return {
      reply: [
  ...
  if (!state.selectedColor) {
    return {
      reply: [
        ...
        "",
        ...config.colors.map((item) => `- ${item.name}${item.premium ? `：加价 ${item.premium} 万元左右` : "：标准色"}`),
        "",
        "如果你不纠结，我可以直接先按标准色给你出一版配置单。",
      ].join("\n"),
      nextActions: config.colors.slice(0, 4).map((item) => `外观选 ${item.name}`),
      missingInfo: ["外观颜色"],
    };
  }
  ...
  if (!state.selectedInterior) {
    return {
      reply: [
        ...
        "",
        ...config.interiors.map((item) => `- ${item.name}${item.premium ? `：加价 ${item.premium} 万元左右` : ""}`),
      ].join("\n"),
      nextActions: config.interiors.slice(0, 3).map((item) => `内饰选 ${item.name}`),
      missingInfo: ["内饰颜色"],
    };
  }
  ...
  if (!state.selectedPackages.length) {
    return {
      reply: [
        ...
        "",
        ...config.packages.map((item) => `- ${item.name}：${formatPrice(item.price)}，${item.desc}`),
        "",
        "如果你想要一版更稳妥的默认方案，我可以直接给你推荐适合当前场景的套件组合。",
      ].join("\n"),
      nextActions: [...config.packages.slice(0, 2).map((item) => `加上 ${item.name}`), "先不加装，直接出配置单"],
      missingInfo: ["套件选择"],
    };
  }
...
function shouldCompleteConfig(state, message) {
  if (!state.selectedModel || !state.selectedVariant || !state.selectedColor || !state.selectedInterior) {
    return false;
  }
...
function buildAgentPayload({ state, stage, message, durationMs, nextActions, missingInfo }) {
  ...
  const confidence = completedSteps / 6;
...
async function runConfiguratorTurn({ message, session }) {
  ...
  if (!hasConfiguratorIntent(message) && !session.configState.selectedModel) {
    return {
      reply:
        "可以。我可以按车型、版本、颜色、内饰和套件一步步帮你完成配置。你先告诉我想配置哪一款小鹏车型，比如 G6、G9、P7i 或 MONA M03。",
      mode: "configurator",
      configSummary: null,
